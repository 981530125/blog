...
$(function(){
	...
	$("input[name='username']").blur(function(){
		...
		$.post("addajax",{username:username.val().trim()},function(stat){
			if(stat==1){
				validate.username=0;
				username.parent().find("span").remove().end().append("<span class='error'>已存在,不可注册</span>");
			}else{
				username.parent().find("span").remove().end().append("<span class='error'>用户可注册</span>");
			}
